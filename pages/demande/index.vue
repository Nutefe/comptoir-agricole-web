...
export default {
  name: 'DemandePage',
  components: { DemandeCard, DemandeCreate },
  ...
  async fetch() {
    this.loading = true
    try {
      await this.$store.dispatch('demandeValide/fetchDemandes', 1)
    } catch (err) {
      this.$nuxt.error({
        statusCode: 503,
  ...
  computed: {
    ...
    ...mapState({
      demandes: (state) => state.demandeValide.demandes,
    }),
  },
  ...
  methods: {
    ...
    async fetchData(page) {
      ...
      try {
        if (this.query) {
          await this.$store.dispatch('demandeValide/searchDemandes', {
            page,
            search: this.query,
          })
        } else {
          await this.$store.dispatch('demandeValide/fetchDemandes', page)
        }
      } catch (err) {
        this.$nuxt.error({
          statusCode: 503,
